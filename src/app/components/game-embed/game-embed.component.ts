...
export class GameEmbedComponent {
  ...
  constructor() {
    this.selectGame('Stickman Destruction');
  }
  ...
  selectGame(game: string) {
    const selectedGame = this.gamesList.find((g) => g.title === game);

    if (!selectedGame) {
      return;
    }

    this.title = selectedGame.title;
    this.embed = selectedGame.embed;
    this.image = selectedGame.image;
    this.tags = selectedGame.tags;
